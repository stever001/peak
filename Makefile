...
validate-phase71: ## Run the Phase 71 R1/R9 evidence-readiness plan check (offline; planning-only, no DB)
	$(PYTHON) tests/validate_phase71_r1_r9_evidence_readiness_plan.py

validate-phase72: ## Run the Phase 72 R10 location model answer set source ingestion check (offline; no credentials/network)
	$(PYTHON) tests/validate_phase72_r10_location_model_answer_set_source_ingestion.py

# Phase 84 checks the Alembic lab/production migration target guard. Fully offline: every URL it
# exercises is synthetic, it contacts no database, sources no env file, and runs no migration
# against any live target.
validate-phase84: ## Run the Phase 84 Alembic target guard check (offline; synthetic URLs only)
	$(PYTHON) tests/validate_phase84_alembic_target_guard.py

validate-phase89: ## Run the Phase 89 lab writer enablement gate check (offline; synthetic URLs only)
	$(PYTHON) tests/validate_phase89_lab_writer_enablement_gate.py

validate-phase90: ## Run the Phase 90 lab engagement anchor bootstrap check (offline; synthetic URLs only)
	$(PYTHON) tests/validate_phase90_lab_engagement_anchor_bootstrap.py

validate-phase110: ## Run the Phase 110 packet-view assembler check (pure; no database)
	$(PYTHON) tests/validate_phase110_packet_view.py

validate-phase111: ## Run the Phase 111 packet-view reporting bridge check (pure; no database)
	$(PYTHON) tests/validate_phase111_packet_view_reporting_bridge.py

validate-phase112: ## Run the Phase 112 planner target-specific review support check (pure; no database)
	$(PYTHON) tests/validate_phase112_planner_target_specific_review_support.py

validate-phase113: ## Run the Phase 113 persisted-state reporting path check (pure; no database)
	$(PYTHON) tests/validate_phase113_persisted_state_reporting_path.py

validate-phase201: ## Run the Phase 201 consultant web auth check (temporary SQLite; needs requirements-web.txt)
	$(PYTHON) tests/validate_phase201_consultant_web_auth.py

validate-phase202: ## Run the Phase 202 client/engagement workspace check (temporary SQLite; needs requirements-web.txt)
	$(PYTHON) tests/validate_phase202_client_engagement_crud.py

validate-phase204: ## Run the Phase 204 discovery/interview workflow check (temporary SQLite; needs requirements-web.txt)
	$(PYTHON) tests/validate_phase204_discovery_workflow.py

validate-phase206: ## Run the Phase 206 discovery-to-assessment integration check (offline; temporary SQLite only)
	$(PYTHON) tests/validate_phase206_discovery_assessment_integration.py
...
lab-writer-enablement-decision-gate: ## OFFLINE lab-only writer enablement decision self-test (synthetic values; contacts nothing)
	$(PYTHON) tools/lab_writer_enablement_decision_gate.py --self-test
