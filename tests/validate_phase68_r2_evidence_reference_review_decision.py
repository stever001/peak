#!/usr/bin/env python3
"""Phase 68 R2 evidence reference review decision check.

Phase 67 created one `evidence_references` row scoped to **item-master source availability and data
readiness only**. Phase 68 records **one** internal review decision on that evidence reference,
approving it for use in a **future internal assessment finding** about that same narrow scope — and
for nothing wider. The evidence stays low-confidence and non-authoritative; no inventory accuracy
conclusion and no SKU/location quantity reliability claim is approved; R1's location dimension stays
provisional pending R9; R8 stays provisional; R3-R7 stay deferred; and report drafting, capsule
publication, and AgentNet resolver publication stay unauthorized.
...
never opened by this harness, and the Phase 68 operator opens no artifact at all — its findings are
...
BASELINE_COMMIT = "69c7041"   # Add Phase 67 first internal test evidence reference

TOOL_REL = "tools/create_internal_test_r2_evidence_review_decision.py"
HARNESS_REL = "tests/validate_phase68_r2_evidence_reference_review_decision.py"
DOC_REL = "docs/PHASE68_R2_EVIDENCE_REFERENCE_REVIEW_DECISION.md"
PHASE67_DOC_REL = "docs/PHASE67_FIRST_INTERNAL_TEST_EVIDENCE_REFERENCE.md"
...
EVIDENCE_ID = "evid_56437d9b9c764560"
R2_PACKET_REF = "pkt_internal_test_r2_sku_item_master_001"
#: Records the operator must never be able to retarget. Phase 68 reviews the evidence reference
#: only, so **no** other stored record id may appear in the tool at all.
FORBIDDEN_TARGET_IDS = ("ing_884c94df03c34908",   # R2 source ingestion
                        "ing_a2abb497f471458e",   # R1
                        "ing_4fb70519cbf84401",   # R8
                        "intn_b8b86b8c196c4595",  # intake note
                        "rev_b82ff6f00790418f",   # intake review decision
                        "rev_bf7f18a13d8f461c")   # R2 source ingestion review decision
...
def tmpdir() -> str:
    tmp = tempfile.mkdtemp(prefix="peak_phase68_")
    _tmpdirs.append(tmp)
    return tmp


def temp_sqlite_url() -> str:
    return "sqlite:///" + os.path.join(tmpdir(), "phase68.db")


def synthetic_packet_hash(salt: str = "a") -> str:
    """A throwaway hash built at runtime. No real artifact is opened by this harness."""
    return hashlib.sha256(("phase68-synthetic-fixture-" + salt).encode("utf-8")).hexdigest()
...
def baseline_checks() -> None:
    ...
    if not is_ancestor:
        print("        reason: phase68_baseline_commit_not_ancestor")
    ...
    check("no migration 015 or later - Phase 68 adds no migration",
...
def writer_contract_checks() -> None:
    print("\n2. The existing review writer honestly represents an evidence-reference review")
    ...
    check("the review writer persists the findings list, so limits are stored as findings",
          '"reasons": list(draft.reasons or [])' in writer_src)

    # The reviewed target column exists and is distinct from the anchor.
    from peak.db.models import ReviewRecord
    cols = set(ReviewRecord.__table__.columns.keys())
    check("review_records carries a reviewed-target column distinct from the anchor",
          {"target_id", "subject_record_type"} <= cols and "engagement_id" in cols)
    check("review_records carries the decision and authoritative columns",
          {"decision", "authoritative"} <= cols)
...
def packet_checks() -> None:
    print("\n3. The one decision packet reviews the R2 evidence reference only")
    import create_internal_test_r2_evidence_review_decision as tool
    ...
    check("the reviewed target is the R2 evidence reference",
          tool.SUBJECT_RECORD_ID == EVIDENCE_ID)
    check("the reviewed target type names the stored evidence_references row",
          tool.SUBJECT_RECORD_TYPE == "evidence_reference")
    check("the reviewed packet reference is R2's", tool.SOURCE_REFERENCE_ID == R2_PACKET_REF)
    ...
    check("the idempotency key is Phase 68 scoped",
          tool.IDEMPOTENCY_KEY.startswith("phase68_"))

    reasons = list(tool.REASONS)
    joined = " ".join(reasons)
    low = joined.lower()
    check("findings identify the reviewed target as an evidence reference",
          any(r.startswith("reviewed_target:") and "evidence_reference" in r for r in reasons))
    check("findings record the Phase 66 -> Phase 67 chain the review rests on",
          any(r.startswith("chain:") and "phase66" in r and "phase67" in r for r in reasons))
    check("findings confirm the evidence did not exceed the approved scope",
          any(r.startswith("scope_check:") and "item-master source availability" in r
              and "data readiness" in r for r in reasons))
    check("findings are sanitized structural counts, not artifact prose",
          any(r.startswith("structure:") for r in reasons))
    check("findings record that the evidence remains low-confidence",
          any(r.startswith("confidence:") and "low" in r for r in reasons))
    check("findings record the reviewed record's needs_review / draft posture",
          any(r.startswith("posture_check:") and "needs_review" in r and "draft" in r
              for r in reasons))
    ...
    check("findings record the missing typed provenance link as a known limitation",
          any(r.startswith("provenance_note:") for r in reasons))

    check("the decision approves only future internal assessment use of the narrow scope",
          any(r.startswith("decision_meaning:") and "internal assessment" in r
    ...
    check("the decision does not authorize SKU or location quantity reliability claims",
          any(r.startswith("not_authorized:") and "quantity reliability" in r for r in reasons))
    check("the decision does not validate R1 location claims and names R9",
          any(r.startswith("not_authorized:") and "R1 location claims" in r and "R9" in r
              for r in reasons))
    check("the decision does not confirm R8 authority precedence",
          any(r.startswith("not_authorized:") and "R8 authority precedence" in r
    ...
    check("the decision leaves R3-R7 deferred",
          any(r.startswith("not_authorized:") and "R3-R7" in r and "deferred" in r
              for r in reasons))
    check("the decision does not authorize report drafting or capsule publication",
          any(r.startswith("not_authorized:") and "report drafting" in r
              and "capsule publication" in r for r in reasons))
    ...
    check("findings name R9 as the likely next collection step",
          any(r.startswith("next_step:") and "R9" in r for r in reasons))
    check("findings carry no row-like item/quantity/location content",
          not ROW_LIKE_RE.search(joined))
    check("findings name no stored record id other than the reviewed evidence reference",
          not any(rid in joined for rid in FORBIDDEN_TARGET_IDS))
    check("findings carry no home-directory path", os.path.expanduser("~") not in joined)
    assert low  # the lowercased blob is used by the checks above

    request = tool.build_request()
    draft = request.record_draft
    check("the draft reviews the evidence reference and carries R2's packet reference",
          draft.subject_record_id == EVIDENCE_ID
...
def tool_checks() -> None:
    ...
    check("utility creates no Client, Engagement, intake, source, evidence, report, or capsule "
          ...
          not re.search(r"(?i)\bClient\(|\bEngagement\(|IntakeNoteDraft|SourceIngestionDraft|"
                        r"EvidencePersistenceDraft|persist_intake|persist_source|persist_evidence|"
    ...
    check("the reviewed target is a module constant, not derived from a flag",
          re.search(r"(?m)^SUBJECT_RECORD_ID\s*=\s*[\"']" + EVIDENCE_ID + r"[\"']", src)
          is not None)
    check("utility names no other stored record id anywhere - it cannot retarget the R2/R1/R8 "
          "source ingestion records, the intake note, or any earlier review",
    ...
    check("utility assigns the reviewed target type only from its own constant",
          re.search(r"subject_record_type\s*=\s*SUBJECT_RECORD_TYPE", code) is not None)
    ...
    check("dry-run prints the reviewed evidence reference and packet reference",
          EVIDENCE_ID in run.stdout and R2_PACKET_REF in run.stdout)
...
def writer_checks() -> None:
    ...
    import create_internal_test_r2_evidence_reference as evidence_tool
    import create_internal_test_r2_evidence_review_decision as tool
    ...
    # Rebuild the anchor, the Phase 65 R2 registration, and the Phase 67 evidence reference,
    # each through its own writer.
    ...
    evidence = persist_evidence_reference(evidence_tool.build_request(), session_factory=factory)
    check("the anchor, the R2 registration, and the evidence reference exist to be reviewed",
          anchor.outcome == "created" and r2.outcome == "created"
          and evidence.outcome == "created")

    # Point the request at the evidence reference this fixture actually created.
    request = tool.build_request()
    request.record_draft.subject_record_id = evidence.stored_record_id
    ...
    check("the review record targets the R2 evidence reference",
          row.target_id == evidence.stored_record_id
          and row.subject_record_type == "evidence_reference")
    ...
    check("the stored detail records its Phase 68 provenance",
          row.details_json.get("source_phase") == "phase68")
    ...
    # The reviewed evidence reference was not modified by the review.
    reviewed = session.get(EvidenceReference, evidence.stored_record_id)
    check("the reviewed evidence reference itself was not modified by the review",
          reviewed.review_status == "needs_review" and reviewed.output_status == "draft"
          and reviewed.reliability == "low")

    stored_reasons = row.details_json["reasons"]
    check("the stored findings approve only future internal assessment use of the narrow scope",
          any(r.startswith("decision_meaning:") and "internal assessment" in r
              and "item-master source availability" in r for r in stored_reasons))
    ...
    check("the stored findings refuse SKU/location quantity reliability claims",
          any("quantity reliability" in r for r in stored_reasons))
    check("the stored findings leave R1 and R8 provisional",
          any("R1 location claims" in r for r in stored_reasons)
          and any("R8 authority precedence" in r for r in stored_reasons))
    check("the stored findings keep the evidence low-confidence",
          any(r.startswith("confidence:") and "low" in r for r in stored_reasons))
    ...
    check("still exactly one evidence reference - the review created none",
    ...
    replay_request.record_draft.subject_record_id = evidence.stored_record_id
    ...
    conflict_request.record_draft.subject_record_id = evidence.stored_record_id
...
def doc_checks() -> None:
    print("\n6. Docs state what was reviewed, what it authorizes, and what stays shut")
    ...
    f = flat(doc)
    for phrase, label in (
        ("one review_records row", "exactly one review_records row is described"),
        ("evidence reference", "the reviewed target is an evidence reference"),
        ("approve_internal", "the decision is named"),
        ("internal assessment", "the approved future use is named"),
        ("item-master source availability", "the evidence scope is named"),
        ("data readiness", "data readiness is named as the evidence scope"),
        ("low confidence", "the evidence remains low confidence"),
        ("non-authoritative", "the evidence remains non-authoritative"),
        ("no inventory accuracy conclusion", "no inventory accuracy conclusion was made"),
        ("needs_review", "R8 remains needs_review"),
        ("provisional", "R1's location dimension remains provisional"),
        ("r9", "R9 is named as the likely next collection step"),
        ("phase 69", "Phase 69 is named"),
        ...
        (EVIDENCE_ID, "the reviewed evidence reference id is named"),
    ):
    ...
    for rel in ("docs/IMPLEMENTATION_PLAN.md", "docs/DATABASE_ACCESS_AND_AUDIT.md",
                "docs/DATABASE_SCAFFOLD.md", PHASE67_DOC_REL, PHASE66_DOC_REL):
        blob = flat(read(rel))
        name = os.path.basename(rel)
        check(f"{name} records Phase 68", "phase 68" in blob)
        check(f"{name} states the evidence remains non-authoritative",
              "non-authoritative" in blob or "authoritative=false" in blob.replace(" ", ""))

    mk = read("Makefile")
    check("Makefile declares validate-phase68", "validate-phase68" in mk)
    check("validate depends on validate-phase68",
          re.search(r"^validate:.*validate-phase68", mk, re.MULTILINE) is not None)
    ...
    check("the record-creation utility is not wired into validate",
          "create_internal_test_r2_evidence_review_decision" not in mk)
...
def main() -> int:
    print("Peak Phase 68 R2 evidence reference review decision check")
    print("=" * 70)
    try:
        baseline_checks()
        ...
        try:
            ...
            print("         Run: make validate-phase68 PYTHON=.venv/bin/python")
        else:
