#!/usr/bin/env python3
"""Phase 65 internal test R2 and R1 source ingestion records check.

Phase 64 planned the R1-R7 artifact collection and named the Phase 65 execution order: register
**R2 first** (the only request the Phase 63 R8 map shows as unblocked, and the interpretive key for
R1), then **R1** with its **location dimension explicitly provisional**. Phase 65 registers those
two artifacts as **two** metadata-only source ingestion records through the unchanged Phase 24
writer.
...
**No artifact body lives in this repository and none is read here.** Neither external artifact is
...
Layers: baseline - writer contract - packets - operator utility - writer behaviour - docs -
isolation.
...
BASELINE_COMMIT = "65e8fb8"   # Add Phase 64 internal test R1-R7 source artifact plan

TOOL_REL = "tools/create_internal_test_r1_r2_source_ingestion_records.py"
PHASE63_TOOL_REL = "tools/create_internal_test_source_ingestion_record.py"
HARNESS_REL = "tests/validate_phase65_r1_r2_source_ingestion_records.py"
DOC_REL = "docs/PHASE65_R1_R2_INTERNAL_TEST_SOURCE_INGESTION.md"
PLAN_REL = "docs/PHASE64_INTERNAL_TEST_R1_R7_SOURCE_ARTIFACT_COLLECTION_PLAN.md"
PHASE63_DOC_REL = "docs/PHASE63_FIRST_INTERNAL_TEST_SOURCE_INGESTION.md"
...
#: Phase 65 registers exactly two packets, in this order and no other.
EXPECTED_PACKET_ORDER = ("R2", "R1")

#: The writer's own forbidden draft attributes - none may appear on either operator draft.
FORBIDDEN_DRAFT_ATTRS = ("packet_payload", "raw_packet_content", "raw_content", "payload")

#: The artifact bodies must never be committed. Their filenames must appear nowhere in the repo.
...
def tmpdir() -> str:
    tmp = tempfile.mkdtemp(prefix="peak_phase65_")
    _tmpdirs.append(tmp)
    return tmp


def temp_sqlite_url() -> str:
    return "sqlite:///" + os.path.join(tmpdir(), "phase65.db")


def synthetic_packet_hash(salt: str) -> str:
    """A throwaway hash built at runtime. The real artifacts are never opened by this harness."""
    return hashlib.sha256(("phase65-synthetic-fixture-" + salt).encode("utf-8")).hexdigest()
...
def baseline_checks() -> None:
    ...
    if not is_ancestor:
        print("        reason: phase65_baseline_commit_not_ancestor")
    ...
    check("the Phase 63 operator utility was not modified by this phase",
          not git("diff", "--name-only", "HEAD", "--", PHASE63_TOOL_REL))
...
def packet_checks() -> None:
    print("\n3. Exactly two packets, R2 first, metadata-only, internal-only, and non-final")
    import create_internal_test_r1_r2_source_ingestion_records as tool

    check("the utility declares exactly two packets", len(tool.PACKETS) == 2)
    check("the declared order is R2 then R1",
          tuple(p["key"] for p in tool.PACKETS) == EXPECTED_PACKET_ORDER)
    check("both packets are anchored to engagement "
          f"{ANCHOR_ID} / client {RESERVED_CLIENT_ID}",
    ...
    check("the two idempotency keys are distinct and Phase 65 scoped",
          len({p["idempotency_key"] for p in tool.PACKETS}) == 2
          and all(p["idempotency_key"].startswith("phase65_") for p in tool.PACKETS))
    check("the two packet reference ids are distinct",
          len({p["packet_reference_id"] for p in tool.PACKETS}) == 2)
    check("the two artifact filenames are distinct and match the approved names",
          {p["artifact_name"] for p in tool.PACKETS} == set(ARTIFACT_BASENAMES))

    by_key = {p["key"]: p for p in tool.PACKETS}
    check("R2 is the SKU/item master packet",
          "r2" in by_key["R2"]["packet_reference_id"].lower()
          and "item_master" in by_key["R2"]["packet_reference_id"].lower())
    check("R1 is the current-inventory by SKU/location packet",
          "r1" in by_key["R1"]["packet_reference_id"].lower()
          and "inventory" in by_key["R1"]["packet_reference_id"].lower())

    for key, packet in by_key.items():
        ref = packet["packet_location_reference"]
        check(f"{key} stored location reference is logical, not a filesystem path",
              ref.startswith("internal-test-artifact://phase65/")
              and not ref.startswith("/") and "\\" not in ref
              and os.path.expanduser("~") not in ref)

    r2_reasons = list(by_key["R2"]["reasons"])
    check("R2 provenance notes cite the Phase 64 plan and R2",
          any("PHASE64" in r and "R2" in r for r in r2_reasons))
    check("R2 provenance notes state why R2 is registered first",
          any("unblocked" in r for r in r2_reasons))
    check("R2 provenance notes state R2 is the interpretive key for R1",
          any("interpretive key for R1" in r for r in r2_reasons))
    check("R2 provenance notes name the taxonomy categories",
          any("03_item_sku_master" in r and "09_data_exports" in r for r in r2_reasons))

    r1_reasons = list(by_key["R1"]["reasons"])
    check("R1 provenance notes cite the Phase 64 plan and R1",
          any("PHASE64" in r and "R1" in r for r in r1_reasons))
    check("R1 provenance notes state the location dimension is provisional",
          any(r.startswith("provisional:") and "location dimension is provisional" in r
              for r in r1_reasons))
    check("R1 provenance notes record degraded reliability for location-attributed claims",
          any("degraded reliability" in r and "location-attributed" in r for r in r1_reasons))
    check("R1 provenance notes record the unconfirmed R8 location/WMS posture",
          any(r.startswith("unconfirmed:") and "location/WMS" in r for r in r1_reasons))
    check("R1 provenance notes record the dependency on the R2 item master",
          any(r.startswith("dependency:") and "R2 item master" in r for r in r1_reasons))
    check("R1 provenance notes state it is registered after R2 and why",
          any("registered after R2" in r for r in r1_reasons))

    common = list(tool.COMMON_REASONS)
    check("shared provenance notes state the internal-only, no-real-client-data posture",
          any("no real client data" in r and "not client-facing" in r for r in common))
    check("shared provenance notes state the metadata-only content rule",
          any(r.startswith("content_rule:") and "metadata only" in r for r in common))
    check("shared provenance notes withhold evidence/report/capsule/publication authority",
          any(r.startswith("not_authorized:") and "publication" in r for r in common))
    check("shared provenance notes disclose the artifacts are Peak-authored, not client-supplied",
          any("not a client-supplied export" in r for r in common))
    check("shared provenance notes record that R8 remains needs_review and non-authoritative",
          any("needs_review" in r and "authoritative=false" in r for r in common))

    from peak.db.source_ingestion_writer import _pre_db_validate
    for index, packet in enumerate(tool.PACKETS):
        request = tool.build_request(packet, synthetic_packet_hash(f"packet{index}"))
        draft = request.record_draft
        key = packet["key"]
        check(f"{key} draft carries no forbidden raw payload attribute",
              not any(hasattr(draft, a) for a in FORBIDDEN_DRAFT_ATTRS))
        check(f"{key} draft leaves server-controlled fields unset",
              draft.source_ingestion_record_id is None and draft.created_at is None)
        check(f"{key} draft posture is draft / needs_review / active",
              draft.output_status == "draft" and draft.review_status == "needs_review"
              and draft.lifecycle_status == "active")
        check(f"{key} draft is non-authoritative, non-client-facing, not capsule-ready",
              draft.authoritative is False and draft.client_facing_approved is False
              and draft.capsule_candidate_ready is False)
        check(f"{key} authorization anchor is the engagement, never the packet",
              request.subject.subject_record_type == "engagement"
              and request.subject.subject_record_id == ANCHOR_ID)
        check(f"{key} request targets source_ingestion_records / "
        ...
        check(f"{key} packet passes the writer's pre-DB governance gate (no connection opened)",
...
def tool_checks() -> None:
    print("\n4. The operator utility writes two records in a fixed order and can express no other")
    src = read(TOOL_REL)
    code = code_only(src)
    ...
    check("utility never reorders or extends its fixed packet order",
          not re.search(r"(?:sorted|reversed)\(\s*PACKETS|PACKETS\s*\.\s*(?:append|extend|"
                        r"insert|sort|reverse)|PACKETS\s*\+", code))
    check("utility hashes both artifacts before any write is attempted",
          re.search(r"prepared\.append", code) is not None
          and code.index("prepared.append") < code.index("if not args.execute:"))

    args = set(re.findall(r'add_argument\("(--[a-z0-9-]+)"', src))
    check("utility exposes only run mode and the two artifact paths - no record field is a flag",
          args <= {"--dry-run", "--execute", "--r2-artifact-path", "--r1-artifact-path"})
    ...
    check("dry-run reports no connection was made",
          run.stdout.count("database_connection_made              : False") == 2)
    check("dry-run reports nothing was written", "DRY-RUN PASS" in run.stdout)
    check("dry-run validates both packets",
          "[ok] the R2 packet passes" in run.stdout
          and "[ok] the R1 packet passes" in run.stdout)
    check("dry-run presents R2 before R1",
          run.stdout.index("[1/2] R2") < run.stdout.index("[2/2] R1"))
    ...
    check("dry-run prints both logical location references, not filesystem paths",
          "internal-test-artifact://phase65/r2-sku-item-master-export-v1" in run.stdout
          and "internal-test-artifact://phase65/r1-current-inventory-sku-location-v1"
          in run.stdout)
    check("dry-run records the R1 location dimension as provisional",
          "location dimension is provisional" in run.stdout)
    # Artifact-agnostic: both artifacts are JSON, so any leak would carry object syntax. Matching
    # on their own key names would instead copy their internals into this repository.
    check("dry-run prints no JSON object syntax - neither artifact body can have leaked",
    ...
    check("dry-run prints no home-directory path outside the clearly-labelled artifact lines",
          not re.search(r"(?m)^(?!.*artifact path).*" + re.escape(os.path.expanduser("~")),
                        run.stdout))

    for flag in ("--r2-artifact-path", "--r1-artifact-path"):
        inside = run_tool(flag, os.path.join(REPO_ROOT, "README.md"))
        check(f"utility refuses an artifact path inside the repository ({flag})",
              inside.returncode == 1
              and "inside the repository" in (inside.stderr + inside.stdout))
        outside = run_tool(flag, os.path.join(tmpdir(), "elsewhere.json"))
        check(f"utility refuses any path other than the approved artifact ({flag})",
    ...
    # real external artifacts are never touched.
    import create_internal_test_r1_r2_source_ingestion_records as tool
    ...
    try:
        ...
        packet = tool.PACKETS[0]
        try:
            tool.resolve_artifact_path(packet, None)
        ...
        empty = os.path.join(fake_dir, packet["artifact_name"])
        with open(empty, "wb"):
            pass
        try:
            tool.artifact_metadata(packet, tool.resolve_artifact_path(packet, None))
...
def writer_checks() -> None:
    print("\n5. The writer creates exactly two records, R2 before R1, and nothing else")
    ...
    check("the engagement anchor exists for the ingestions to be written under",
          anchor.outcome == "created")

    hashes = {p["key"]: synthetic_packet_hash(p["key"]) for p in tool.PACKETS}
    receipts = {}

    # Driven by the utility's own declared order, so this proves the tool's ordering rather than
    # restating it: after the Nth write there must be exactly N rows, and the newest must be the
    # Nth declared packet.
    for index, packet in enumerate(tool.PACKETS):
        key = packet["key"]
        receipt = persist_source_ingestion_record(
            tool.build_request(packet, hashes[key]), session_factory=factory)
        receipts[key] = receipt
        check(f"{key} invocation creates a source ingestion record", receipt.outcome == "created")
        check(f"{key} receipt reports one stored record created",
              receipt.stored_record_created is True)
        check(f"{key} receipt reports the record is review-gated",
              receipt.review_status == "needs_review" and receipt.output_status == "draft")

        session = factory()
        check(f"exactly {index + 1} source ingestion record(s) exist after {key}",
              session.query(SourceIngestionRecord).count() == index + 1)
        row = session.get(SourceIngestionRecord, receipt.stored_record_id)
        check(f"{key} row carries this packet's reference id",
              row.source_reference_id == packet["packet_reference_id"])
        session.close()

    check("R2 was inserted before R1 - after the first write only R2 existed",
          tool.PACKETS[0]["key"] == "R2"
          and receipts["R2"].stored_record_id != receipts["R1"].stored_record_id)

    session = factory()
    check("exactly two source ingestion records exist in total",
          session.query(SourceIngestionRecord).count() == 2)

    for packet in tool.PACKETS:
        key = packet["key"]
        row = session.get(SourceIngestionRecord, receipts[key].stored_record_id)
        check(f"{key} record is tied to the internal_test engagement",
        ...
        check(f"{key} ingesting engagement is classified internal_test",
              session.get(Engagement, row.engagement_id).engagement_category == "internal_test")
        check(f"{key} stored detail is metadata only - schema, source type, location, hash",
              {"packet_schema_name", "packet_source_type", "packet_location_reference",
               "packet_hash"} <= set(row.details_json))
        check(f"{key} stored hash is the packet hash, not artifact content",
              row.details_json["packet_hash"] == hashes[key])
        check(f"{key} stored location reference is logical, not a filesystem path",
              row.details_json["packet_location_reference"]
              == packet["packet_location_reference"]
              and row.details_json["packet_location_reference"].startswith(
                  "internal-test-artifact://"))
        check(f"{key} row stores no raw artifact payload anywhere",
              not any(k in row.details_json for k in FORBIDDEN_DRAFT_ATTRS))
        check(f"{key} row carries no home-directory path",
              os.path.expanduser("~") not in str(row.details_json))
        check(f"{key} record is non-authoritative and not capsule-ready",
    ...
    r1_row = session.get(SourceIngestionRecord, receipts["R1"].stored_record_id)
    check("the R1 row records the provisional location dimension in its provenance notes",
          any("location dimension is provisional" in r
              for r in r1_row.details_json.get("reasons", [])))
    ...
    # Replay: identical payloads must not write a third or fourth row.
    for packet in tool.PACKETS:
        key = packet["key"]
        replay = persist_source_ingestion_record(
            tool.build_request(packet, hashes[key]), session_factory=factory)
        check(f"{key} identical replay is idempotent, not a second write",
              replay.outcome == "idempotent_replay" and replay.database_write_made is False)
    session = factory()
    check("still exactly two source ingestion records after replaying both",
          session.query(SourceIngestionRecord).count() == 2)
    session.close()
    ...
    for packet in tool.PACKETS:
        ...
        conflict = persist_source_ingestion_record(
            tool.build_request(packet, synthetic_packet_hash(key + "-changed")),
            session_factory=factory)
        check(f"{key} changed artifact hash under the same idempotency key is denied",
              conflict.reason_code == "idempotency_conflict" and conflict.permitted is False)
    session = factory()
    check("still exactly two source ingestion records after both conflicts",
          session.query(SourceIngestionRecord).count() == 2)
    for packet in tool.PACKETS:
        key = packet["key"]
        stored = session.get(SourceIngestionRecord, receipts[key].stored_record_id)
        check(f"{key} existing record was not overwritten",
              stored is not None and stored.details_json["packet_hash"] == hashes[key])
    session.close()
...
def doc_checks() -> None:
    ...
    f = flat(doc)
    for phrase, label in (
        ("two source ingestion records", "exactly two records are described"),
        ("r2 first", "R2 was registered first"),
        ("item master", "the R2 SKU/item master export is a subject"),
        ("current inventory", "the R1 current inventory export is a subject"),
        ("outside the repository", "the artifact bodies live outside the repository"),
        ("metadata", "only metadata was persisted"),
        ("packet_hash", "the hash is named as what was persisted"),
        ("internal-test-artifact://", "the logical location references are recorded"),
        ("provisional", "the R1 location dimension remains provisional"),
    ...
    for rel in ("docs/IMPLEMENTATION_PLAN.md", "docs/DATABASE_ACCESS_AND_AUDIT.md",
                "docs/DATABASE_SCAFFOLD.md", PLAN_REL, PHASE63_DOC_REL):
        blob = flat(read(rel))
        name = os.path.basename(rel)
        check(f"{name} records Phase 65", "phase 65" in blob)
    ...
    mk = read("Makefile")
    check("Makefile declares validate-phase65", "validate-phase65" in mk)
    check("validate depends on validate-phase65",
          re.search(r"^validate:.*validate-phase65", mk, re.MULTILINE) is not None)
    ...
    check("the record-creation utility is not wired into validate",
          "create_internal_test_r1_r2_source_ingestion_records" not in mk)
...
def isolation_checks() -> None:
    ...
    check("this harness never opens either external artifact",
          not any(name in code for name in ARTIFACT_BASENAMES)
          and "peak-internal-test-artifacts" not in code)
    check("this harness's packet hashes are synthetic and built at runtime",
...
def main() -> int:
    print("Peak Phase 65 internal test R2 and R1 source ingestion records check")
    print("=" * 70)
    try:
        baseline_checks()
        ...
        try:
            ...
            print("         Run: make validate-phase65 PYTHON=.venv/bin/python")
        else:
