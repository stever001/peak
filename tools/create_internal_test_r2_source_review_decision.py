#!/usr/bin/env python3
"""Phase 66 — record the **one** internal review decision on the Phase 65 R2 source ingestion record.
...
particular there is **no flag that can retarget R1, R8, the intake note, or any other record** —
the reviewed target is a module constant, no other stored record id appears anywhere in this file,
...
anchor"). ``subject_record_type`` is exactly the column that "disambiguates the reviewed target",
so ``source_ingestion_record`` is an honest value in it, not a reuse of a field meant for something
else. ``draft.source_reference_id`` carries the reviewed packet reference. The Phase 61 intake
review used this same shape with ``subject_record_type='intake_note'``.

The alternative, ``internal_reviewer_decision_records``, is shaped around a **review bundle** and
has no single reviewed-target field, so representing one source ingestion record there would mean
misusing a bundle reference. ``review_records`` is the narrower honest fit.
...
approval**, which is exactly the finding: R2 is sufficient to proceed to a **future
``evidence_reference`` about item-master source availability and data readiness** — and to nothing
wider.

**What this decision deliberately does not authorize.** It does not authorize an inventory accuracy
conclusion (R2 describes an item master, not measured on-hand quantity); it leaves **R1 provisional**
(its location dimension depends on an unconfirmed location/bin model) and **R8 provisional**
(``needs_review`` / ``draft`` / ``authoritative=false``, authority precedence rule unconfirmed); it
leaves **R3-R7 deferred**; and it authorizes **no report drafting, no capsule candidacy, and no
AgentNet resolver publication**. ``ReviewRecordDraft`` has no ``publication_allowed`` or
``execution_allowed`` field to set false — the prohibition is structural instead, and stronger: the
writer refuses ``publish_capsule`` at the vocabulary level and forces
``client_facing_approved=false`` and ``capsule_candidate_ready=false``. The limits are additionally
recorded in the ``reasons`` findings so the stored row states them in its own text.

**``authoritative`` is left false.** The writer would permit ``true`` for ``approve_internal``, but
this decision reviews a source registration whose upstream map (R8) is itself unreviewed. Nothing
downstream should yet treat it as settled.

**Findings, not prose.** The ``reasons`` list carries concise sanitized findings — field counts,
posture flags, and named gaps. It carries **no artifact body text, no field values, no item or SKU
values, no quantities, and no location identifiers**. The R2 artifact body stays outside the
...
#: The Phase 65 R2 source ingestion record — the reviewed target, stored as ``target_id``.
#: A constant, deliberately: no flag can point this at R1, R8, or any other record.
SUBJECT_RECORD_ID = "ing_884c94df03c34908"
...
#: Left false deliberately: R2's upstream map (R8) is itself unreviewed, so nothing downstream
#: should treat this determination as settled. The writer permits either value here.
AUTHORITATIVE = False
...
REASONS = (
    "source_review: R2 SKU/item master export source ingestion record",
    "reviewed_target: source_ingestion_record registered in phase65",
    "reviewed_packet: pkt_internal_test_r2_sku_item_master_001",
    "registration_integrity: reviewed artifact hash matches the registered packet_hash; "
    "the artifact registered in phase65 is unchanged",
    "structure: artifact describes 10 item-master fields - 6 required, 4 optional",
    "structure: every described field carries an explicit interpretation note and a named risk",
    "structure: join key to R1 is the item identifier; 5 fields are required to interpret R1",
    "readiness: R8 records no blocker for R2 - it is the first unblocked source artifact",
    "readiness: artifact is a field-level export description, not an export; it carries no rows",
    "gap: unit-of-measure posture unconfirmed - distinct UoM codes, multi-UoM items, and "
    "conversion factors must be measured before any quantity claim rests on it",
    "gap: item-status posture unconfirmed - master size is not meaningful until obsolete and "
    "blocked rows are separable",
    "gap: 6 duplicate/normalization risks recorded - any duplicate rate must cite its "
    "normalization rule and status filter or it supports a qualitative finding only",
    ...
    "decision_meaning: R2 is sufficient to proceed to a future evidence_reference about "
    "item-master source availability and data readiness",
    "not_authorized: inventory accuracy conclusions - R2 describes an item master, not measured "
    "on-hand quantity",
    "not_authorized: R1 remains provisional - its location dimension depends on an unconfirmed "
    "location/bin model",
    ...
    "authoritative=false",
    "next_step: a single evidence_reference scoped to item-master source availability and data "
    "readiness, as a separately approved phase",
)
...
def execute(request) -> int:
    ...
    if outcome == "created":
        print("\nRESULT: CREATED (exactly one internal review decision on the R2 source "
              "ingestion record)")
        return 0
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description=f"Record the one internal review decision on R2 source ingestion record "
                    ...
                    "item-master source availability / data-readiness evidence reference. "
    ...
    print("Peak Phase 66 — internal review decision on the R2 source ingestion record")
    print("=" * 70)
    emit_packet()
